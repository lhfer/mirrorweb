...
| Optics — O5 source-exact card optical body | **FAILED ABSOLUTE GATE** (8/14). The candidate is the Target's whole material chain in one lane, and it **enters the Target's reflection-band window at all four viewports** (3.3/2.0/2.0/2.0 px against 3.3/1.5/2.0/2.0, from a control at 15.3/8.0/8.0/7.0) — the measurement O3 and O4 both failed. Dark-side luma lands on the Target at three of four. Of the six failures, four are sealed instruments that cannot discriminate (the Target fails item 4 worse than the candidate; item 10 fails the accepted body too). Control identity EXACT 0 across 35 comparisons; compiled audit 15/15; §十 7/7; §十二 18/18. Default stays `opticalBody=current`. See [`qa-v5/optics-o5/`](../../qa-v5/optics-o5/README.md) |
