...
| Optics — O5 source-exact card optical body | **OPEN** — one cohesive candidate lane `opticalBody=target-source`: the Target's plane/position model, rounded-rect SDF alpha, analytic bevel normal, own-media per-IOR refraction, spectral accumulation, level-0 sampling, no adaptive shaping, System B env in the same body, white SDF rim, `toneMapped=false`. Default stays `opticalBody=current` until product review |
