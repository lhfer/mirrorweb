...
| Optics — O5R corrected instruments + source environment | **OPEN** — closure round. Repairs the four unreadable instruments and the two degenerate rows, removes the one non-source deviation (`envSampleCeiling = 16`, which the O5 evidence proved binds), and re-runs the sealed O5 gate unchanged as a regression. May not touch any Target source constant, the HDR asset, layout, typography, motion, culling, media crop, camera or adaptive quality. Default stays `opticalBody=current` |
...
- O5R optics closure: corrected instruments + source environment — the O5 product review + O5R open commit · the instrument contract commit (definitions and the HDR radiance audit, sealed before any O5R candidate pixel) · the source-environment code commit (`envSampleCeiling` removed from the target-source lane; `environmentMode=source|off` structural programs; QA-only refraction and silhouette views) · the product closure evidence commit. Shipped default stays `opticalBody=current`
