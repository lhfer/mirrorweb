#!/usr/bin/env python3
"""O4 gate 12 -- every frozen suite in §十一, re-run at the O3 build.

Emits a flat `suites` list so each of the sixteen named suites is visible
as its own PASS/FAIL rather than folded into a category. O4 changed one BODY subsystem behind a build-time lane whose
default is unchanged; this file is what
says so.
...
def main() -> int:
    ...
    measure = jload(REPO / "artifacts/optics-o4/gate/gate-manifest.json")
    ...
    print("O4 REGRESSIONS:", "PASS" if doc["pass"] else "FAIL")
