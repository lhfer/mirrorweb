#!/usr/bin/env node
/**
 * O5 review recordings -- §九.12's sequences over the Target, the control and
 * the target-source candidate, on the deterministic shared media frozen at the
 * same time the gates measured.
 *
 ...
 * ?opticalBody alone. The Target is recorded through the same driver, so a
 * reviewer comparing three clips is comparing three renders of one gesture.
 *
 * Usage: o5-recording.mjs --local=<origin> [--target=<url>] [--out=<dir>]
...
const opts = { local: null,
  ...
  out: path.join(REPO, "artifacts/optics-o5/recordings"),
  lanes: ["target", "control", "candidate"] };
...
const Q = "?composition=sourceExact&qa&dispersionLaw=o1&reflectionSupport=geometry&bodyFloorMode=current&opticalBody=";
const LANE = {
  target: { url: opts.target, kind: "target" },
  control: { url: `${opts.local}/${Q}current`, kind: "local" },
  candidate: { url: `${opts.local}/${Q}target-source`, kind: "local" },
};
