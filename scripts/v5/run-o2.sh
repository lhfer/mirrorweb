#!/usr/bin/env bash
# O2 System B -- frozen-regression stage runner (F9).
#
...
#   regressions  aggregate everything into qa-v5/optics-o2/regressions.json
#
# Every output goes under artifacts/optics-o2/regressions or qa-v5/optics-o2.
# The SEALED V0/V1/O1 evidence trees are never written.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
REPO="$(cd "$HERE/../.." && pwd)"
OUT="$REPO/qa-v5/optics-o2"
ART="$REPO/artifacts/optics-o2/regressions"
...
stage_rendergate() {
  say "STAGE rendergate -- the V1 absolute gate at the O2 build"
  start_server
...
stage_labelgate() {
  say "STAGE labelgate -- the V0 absolute gate at the O2 build"
  start_server
  rm -f "$CART/candidate-o2/culling-trace.json"
  node "$HERE/v0-culling-trace.mjs" --lane=candidate "--url=$LOCAL_URL" \
    "--out=$CART/candidate-o2" > "$LOGS/labelgate-trace.log" 2>&1 \
  ...
  python3 "$HERE/v0-coverage-truth.py" \
    ...
    "--candidate=$CART/candidate-o2/culling-trace.json" \
...
stage_regressions() {
  ...
  python3 "$HERE/o1-regressions.py" "--art=$ART" \
    "--out=$OUT/regressions.json" || die "regressions aggregate FAILED"
  python3 - "$OUT/regressions.json" <<'APPEND'
import json, sys
p = sys.argv[1]
d = json.load(open(p))
d["instrumentUpdates"] = [{
  "gate": "V1 render culling (v1-render-trace.mjs / v1-render-gate.py)",
  "what": "the shell/glass composition law is MODE-AWARE since O2",
  "why": ("O2 System B turns the separate reflection shell OFF in Beauty on "
          "the source-exact route (pre-registered; the white reflection lives "
          "in the body LERP). The V1-era law asserted shell==glass "
          "unconditionally and fired on every frame of the first O2 re-run "
          "(20219/20219) purely on the intended default change."),
  "newLaw": ("the trace records the applied shellMode as engine truth; with "
             "mode 'off' the gate asserts ZERO shells per frame (catching "
             "shell ink leaking into Beauty -- the O2 concern), with any "
             "other mode the V1 law shell==glass applies unchanged"),
  "firstRunResult": "FAIL 20219/20219 shellGlassMismatches, all other measurands zero",
}]
json.dump(d, open(p, "w"), indent=1)
print("instrument-update note appended")
APPEND
