#!/usr/bin/env bash
# O5F -- frozen-regression stage runner.
#
# Derived from run-o5r.sh with the output roots moved to the O5F tree and
# the aggregation handed to o5f-regressions.py, which imports the sealed
# o5-regressions.py unedited. The capture stages themselves are the same
# suites: that is what makes them a regression.
#
...
#   regressions  aggregate everything into qa-v5/optics-o5f/regressions.json
#
# Every output goes under artifacts/optics-o5f/regressions or qa-v5/optics-o5f.
# The SEALED V0/V1/O1 evidence trees are never written.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
REPO="$(cd "$HERE/../.." && pwd)"
OUT="$REPO/qa-v5/optics-o5f"
ART="$REPO/artifacts/optics-o5f/regressions"
...
stage_rendergate() {
  say "STAGE rendergate -- the V1 absolute gate at the O5F build"
  start_server
...
stage_labelgate() {
  say "STAGE labelgate -- the V0 absolute gate at the O5F build"
  start_server
  rm -f "$CART/candidate-o5f/culling-trace.json"
  node "$HERE/v0-culling-trace.mjs" --lane=candidate "--url=$LOCAL_URL" \
    "--out=$CART/candidate-o5f" > "$LOGS/labelgate-trace.log" 2>&1 \
  ...
  python3 "$HERE/v0-coverage-truth.py" \
    ...
    "--candidate=$CART/candidate-o5f/culling-trace.json" \
...
stage_o2suites() {
  say "STAGE o2suites -- the O2 suites the freeze requires to keep passing"
  start_server
  mkdir -p "$ART/harness"
  ...
  say "  media-only controls (from the O5 scoring captures)"
  python3 - "$REPO/artifacts/optics-o5f/measure" "$ART/media-only-controls.json" <<'MOC'
import json, sys
...
pairs = []
for rec in find(kind="media-only", lane="o5r-unclamped"):
...
doc = {"what": "with the glass layer hidden the two BODY lanes must render "
               "the same media exactly -- the body colour is not in the "
               "media path at all.",
       "pairs": pairs,
...
stage_regressions() {
  ...
  python3 "$HERE/o5f-regressions.py" "--art=$ART" \
