#!/usr/bin/env node
/**
 * O3 review recordings -- §十's four sequences over Target / O2 control /
 * O3 candidate, on the deterministic shared media frozen at the time the
 * gates measured.
 *
...
const opts = { local: null,
  ...
  out: path.join(REPO, "artifacts/optics-o3/recordings"),
  lanes: ["target", "control", "candidate"] };
...
const Q = "?composition=sourceExact&qa&dispersionLaw=o1&reflectionSupport=";
const LANE = {
  target: { url: opts.target, kind: "target" },
  control: { url: `${opts.local}/${Q}geometry`, kind: "local" },
  candidate: { url: `${opts.local}/${Q}target-sdf`, kind: "local" },
};
...
const index = { freeze: 4,
  ...
  lanes: { control: "reflectionSupport=geometry (accepted O2 System B)",
           candidate: "reflectionSupport=target-sdf (O3 analytic bevel)",
           target: opts.target },
