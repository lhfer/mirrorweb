#!/usr/bin/env bash
# O5 source-exact optical body -- frozen-regression stage runner (§十二).
#
...
#   regressions  aggregate everything into qa-v5/optics-o5/regressions.json
#
# Every output goes under artifacts/optics-o5/regressions or qa-v5/optics-o5.
# The SEALED V0/V1/O1 evidence trees are never written.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
REPO="$(cd "$HERE/../.." && pwd)"
OUT="$REPO/qa-v5/optics-o5"
ART="$REPO/artifacts/optics-o5/regressions"
...
stage_o2suites() {
  ...
  python3 - "$REPO/artifacts/optics-o5/measure" "$ART/media-only-controls.json" <<'MOC'
import json, sys
...
stage_regressions() {
  ...
  python3 "$HERE/o5-regressions.py" "--art=$ART" \
