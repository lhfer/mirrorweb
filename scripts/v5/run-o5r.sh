#!/usr/bin/env bash
# O5R closure -- frozen-regression stage runner (§十四).
#
# Derived from run-o5.sh with the output roots moved to the O5R tree and
# the aggregation handed to o5r-regressions.py, which imports the sealed
...
#   regressions  aggregate everything into qa-v5/optics-o5r/regressions.json
#
# Every output goes under artifacts/optics-o5r/regressions or qa-v5/optics-o5r.
# The SEALED V0/V1/O1 evidence trees are never written.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
REPO="$(cd "$HERE/../.." && pwd)"
OUT="$REPO/qa-v5/optics-o5r"
ART="$REPO/artifacts/optics-o5r/regressions"
...
stage_rendergate() {
  say "STAGE rendergate -- the V1 absolute gate at the O5R build"
  start_server
...
stage_labelgate() {
  say "STAGE labelgate -- the V0 absolute gate at the O4 build"
  start_server
  rm -f "$CART/candidate-o5r/culling-trace.json"
  node "$HERE/v0-culling-trace.mjs" --lane=candidate "--url=$LOCAL_URL" \
    "--out=$CART/candidate-o5r" > "$LOGS/labelgate-trace.log" 2>&1 \
  ...
  python3 "$HERE/v0-coverage-truth.py" \
    ...
    "--candidate=$CART/candidate-o5r/culling-trace.json" \
...
stage_o2suites() {
  say "STAGE o2suites -- the O2 suites §十四 requires to keep passing"
  start_server
  mkdir -p "$ART/harness"
  ...
  python3 - "$REPO/artifacts/optics-o5r/measure" "$ART/media-only-controls.json" <<'MOC'
import json, sys
...
stage_regressions() {
  ...
  python3 "$HERE/o5r-regressions.py" "--art=$ART" \
