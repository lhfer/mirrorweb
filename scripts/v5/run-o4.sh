#!/usr/bin/env bash
# O4 frozen body floor -- frozen-regression stage runner (§十一).
#
...
#   regressions  aggregate everything into qa-v5/optics-o4/regressions.json
#
# Every output goes under artifacts/optics-o4/regressions or qa-v5/optics-o4.
# The SEALED V0/V1/O1 evidence trees are never written.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
REPO="$(cd "$HERE/../.." && pwd)"
OUT="$REPO/qa-v5/optics-o4"
ART="$REPO/artifacts/optics-o4/regressions"
...
stage_rendergate() {
  say "STAGE rendergate -- the V1 absolute gate at the O4 build"
  start_server
...
stage_labelgate() {
  ...
  start_server
  rm -f "$CART/candidate-o4/culling-trace.json"
  node "$HERE/v0-culling-trace.mjs" --lane=candidate "--url=$LOCAL_URL" \
    "--out=$CART/candidate-o4" > "$LOGS/labelgate-trace.log" 2>&1 \
  ...
  python3 "$HERE/v0-coverage-truth.py" \
    ...
    "--candidate=$CART/candidate-o4/culling-trace.json" \
...
stage_o2suites() {
  ...
  say "  media-only controls (from the O4 gate captures)"
  python3 - "$REPO/artifacts/optics-o4/gate" "$ART/media-only-controls.json" <<'MOC'
import json, sys
from pathlib import Path
import numpy as np
from PIL import Image
md, out = Path(sys.argv[1]), Path(sys.argv[2])
man = json.loads((md / "gate-manifest.json").read_text())
def find(**kw):
...
stage_regressions() {
  ...
  python3 "$HERE/o4-regressions.py" "--art=$ART" \
